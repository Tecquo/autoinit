...
> ⚠️ **НЕ копируйте `autoinit.rpy` в свой мод.** Для использования в своём моде создайте подкласс `AutoInit` (см. ниже). Исключением являются мобильные порты, ввиду отсутствия удобного установщика зависимостей мода на телефоны.

### Быстрый старт

1. **Поместите `autoinit` в папку `game`**
2. **Поместите свой мод в папку `game` (настоятельно рекомендую, в поиске корневой папки мода предусмотрен поиск мода в подпапках по типу `mods`, но лучше так не делать).**
   - Имя папки мода должно быть уникальным и использоваться как параметр `modID`.
3. **Организуйте ресурсы следующей структурой:**
